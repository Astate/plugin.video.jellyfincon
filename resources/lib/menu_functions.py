...
def displaySections():
    ...
    if collections:
        ...
        addMenuDirectoryItem(string_load(30251), "plugin://plugin.video.jellyfincon/?mode=GENRES&item_type=movie")
        addMenuDirectoryItem(string_load(30252), "plugin://plugin.video.jellyfincon/?mode=MOVIE_ALPHA")
        addMenuDirectoryItem("Movie (Pages)", "plugin://plugin.video.jellyfincon/?mode=MOVIE_PAGES")

        addMenuDirectoryItem(string_load(30289), "plugin://plugin.video.jellyfincon/?mode=GENRES&item_type=tvshow")
        addMenuDirectoryItem(string_load(30246), "plugin://plugin.video.jellyfincon/?mode=SEARCH")

        addMenuDirectoryItem(string_load(30017), "plugin://plugin.video.jellyfincon/?mode=SHOW_SERVER_SESSIONS")
        addMenuDirectoryItem(string_load(30253), "plugin://plugin.video.jellyfincon/?mode=CHANGE_USER")

    addMenuDirectoryItem(string_load(30011), "plugin://plugin.video.jellyfincon/?mode=DETECT_SERVER_USER")
    addMenuDirectoryItem(string_load(30254), "plugin://plugin.video.jellyfincon/?mode=SHOW_SETTINGS")

    # only add these if we have other collection which means we have a valid server conn
    if collections:
        addMenuDirectoryItem(string_load(30293), "plugin://plugin.video.jellyfincon/?mode=CACHE_ARTWORK")
        addMenuDirectoryItem(string_load(30247), "plugin://plugin.video.jellyfincon/?mode=WIDGETS")
    ...
    for item in result:
        ...
        if collection_type == "tvshows":
            ...
            collections.append({
                ...
                'art': art,
                'path': 'plugin://plugin.video.jellyfincon/?mode=GENRES&item_type=tvshow&parent_id=' + item.get("Id"),
                'media_type': 'tvshows'})
        ...
        if collection_type == "movies":
            ...
            collections.append({
                ...
                'item_type': 'plugin_link',
                'art': art,
                'path': 'plugin://plugin.video.jellyfincon/?mode=GENRES&item_type=movie&parent_id=' + item.get("Id"),
                'media_type': collection_type})
            ...
            collections.append({
                ...
                'item_type': 'plugin_link',
                'art': art,
                'path': 'plugin://plugin.video.jellyfincon/?mode=MOVIE_PAGES&parent_id=' + item.get("Id"),
                'media_type': collection_type})
...
def showWidgets():
    ...
    addMenuDirectoryItem(string_load(30324) + " (" + show_x_filtered_items + ")",
                         "plugin://plugin.video.jellyfincon/?mode=GET_CONTENT&use_cache=false&media_type=Movies&url=" + urllib.quote(url))
    ...
    addMenuDirectoryItem(" - " + string_load(30257) + " (" + show_x_filtered_items + ")", 'plugin://plugin.video.jellyfincon/?mode=WIDGET_CONTENT&type=recent_movies')
    addMenuDirectoryItem(" - " + string_load(30258) + " (" + show_x_filtered_items + ")", 'plugin://plugin.video.jellyfincon/?mode=WIDGET_CONTENT&type=inprogress_movies')
    addMenuDirectoryItem(" - " + string_load(30269) + " (" + show_x_filtered_items + ")", 'plugin://plugin.video.jellyfincon/?mode=WIDGET_CONTENT&type=random_movies')

    addMenuDirectoryItem(" - " + string_load(30287) + " (" + show_x_filtered_items + ")", 'plugin://plugin.video.jellyfincon/?mode=WIDGET_CONTENT&type=recent_tvshows')
    addMenuDirectoryItem(" - " + string_load(30263) + " (" + show_x_filtered_items + ")", 'plugin://plugin.video.jellyfincon/?mode=WIDGET_CONTENT&type=recent_episodes')
    addMenuDirectoryItem(" - " + string_load(30264) + " (" + show_x_filtered_items + ")", 'plugin://plugin.video.jellyfincon/?mode=WIDGET_CONTENT&type=inprogress_episodes')
    addMenuDirectoryItem(" - " + string_load(30265) + " (" + show_x_filtered_items + ")", 'plugin://plugin.video.jellyfincon/?mode=WIDGET_CONTENT&type=nextup_episodes')

    xbmcplugin.endOfDirectory(int(sys.argv[1]))


def show_search():
    addMenuDirectoryItem(string_load(30231), 'plugin://plugin.video.jellyfincon/?mode=NEW_SEARCH&item_type=Movie')
    addMenuDirectoryItem(string_load(30229), 'plugin://plugin.video.jellyfincon/?mode=NEW_SEARCH&item_type=Series')
    addMenuDirectoryItem(string_load(30235), 'plugin://plugin.video.jellyfincon/?mode=NEW_SEARCH&item_type=Episode')
    addMenuDirectoryItem(string_load(30337), 'plugin://plugin.video.jellyfincon/?mode=NEW_SEARCH&item_type=Audio')
    addMenuDirectoryItem(string_load(30338), 'plugin://plugin.video.jellyfincon/?mode=NEW_SEARCH&item_type=MusicAlbum')
    addMenuDirectoryItem(string_load(30339), 'plugin://plugin.video.jellyfincon/?mode=NEW_SEARCH&item_type=Person')
...
def set_library_window_values(force=False):
    ...
    for item in result:
        ...
        if type in ["movies", "boxsets", "music", "tvshows"]:
            ...
            log.debug("set_library_window_values: plugin.video.jellyfincon-{0}={1}", prop_name, name)

            prop_name = "view_item.%i.id" % index
            home_window.setProperty(prop_name, id)
            log.debug("set_library_window_values: plugin.video.jellyfincon-{0}={1}", prop_name, id)

            prop_name = "view_item.%i.type" % index
            home_window.setProperty(prop_name, type)
            log.debug("set_library_window_values: plugin.video.jellyfincon-{0}={1}", prop_name, type)
            ...
            log.debug("set_library_window_values: plugin.video.jellyfincon-{0}={1}", prop_name, thumb)
