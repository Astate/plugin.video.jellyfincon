...
class DownloadUtils():
    ...
    def getUserId(self):
        ...
        if userid and userImage:
            log.debug("JellyfinCon DownloadUtils -> Returning saved UserID: {0}", userid)
            return userid
        ...
        if secure or not userid:
            ...
            if authOk == "":
                xbmcgui.Dialog().notification(string_load(30316),
                                              string_load(30044),
                                              icon="special://home/addons/plugin.video.jellyfincon/icon.png")
                return ""
            if not userid:
                userid = WINDOW.getProperty("userid")
    ...
    def authenticate(self):
        ...
        if token is not None and token != "":
            log.debug("JellyfinCon DownloadUtils -> Returning saved AccessToken: {0}", token)
            return token
    ...
    def getAuthHeader(self, authenticate=True):
        ...
        if (authenticate == False):
            ...
            log.debug("JellyfinCon Authentication Header: {0}", headers)
            return headers
