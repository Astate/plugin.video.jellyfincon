...
class Service(xbmc.Player):
    ...
    def onPlayBackStarted(self):
        ...
        if current_playing_file not in self.played_information:
            log.debug("This file was not started by JellyfinCon")
            return
    ...
    def onPlayBackEnded(self):
        ...
        log.debug("JellyfinCon Service -> onPlayBackEnded")
        stopAll(self.played_information)
...
class PlaybackService(xbmc.Monitor):
    ...
    def screensaver_deactivated(self):
        ...
        if show_change_user:
            xbmc.executebuiltin("RunScript(plugin.video.jellyfincon,0,?mode=CHANGE_USER)")
