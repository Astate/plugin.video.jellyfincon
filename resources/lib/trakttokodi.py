...
icon = xbmc.translatePath('special://home/addons/plugin.video.jellyfincon/icon.png')


def not_found(content_string):
    xbmcgui.Dialog().notification('JellyfinCon', string_load(30305) % content_string, icon=icon, sound=False)


def playback_starting(content_string):
    xbmcgui.Dialog().notification('JellyfinCon', string_load(30306) % content_string, icon=icon, sound=False)
...
def entry_point(parameters):
    ...
    if action == 'play':
        ...
        if play_item_id:
            ...
            xbmc.executebuiltin('RunPlugin(plugin://plugin.video.jellyfincon/?mode=PLAY&item_id={item_id})'.format(item_id=play_item_id))
        ...
        if url and media_type:
            xbmc.executebuiltin('ActivateWindow(Videos, plugin://plugin.video.jellyfincon/?mode=GET_CONTENT&url={url}&media_type={media_type})'.format(url=urllib.quote(url), media_type=media_type))
