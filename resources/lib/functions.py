...
def get_params(paramstring):
    ...
    log.debug("JellyfinCon -> Detected parameters: {0}", param)
    return param
...
def show_menu(params):
    ...
    if selected_action == "play":
        ...
        u = ('{server}/emby/Shows/' + series_id +
             ...
             '&format=json')
        action_url = ("plugin://plugin.video.jellyfincon/?url=" + urllib.quote(u) + "&mode=GET_CONTENT&media_type=Season")
        ...
        u = ('{server}/emby/Shows/' + item_id +
             ...
             '&Fields={field_filters}' +
             '&format=json')
        action_url = ("plugin://plugin.video.jellyfincon/?url=" + urllib.quote(u) + "&mode=GET_CONTENT&media_type=Series")
        built_in_command = 'ActivateWindow(Videos, ' + action_url + ', return)'
        xbmc.executebuiltin(built_in_command)
